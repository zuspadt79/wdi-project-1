...
$('.lines').click(function(){
  ...
  clickedDiv.push(parseInt($(this).attr("value")));
...
function checkBox() {
  ...
  for (var i = 0; i < squareCombinations.length; i++) {
    if($(clickedDiv).not(squareCombinations[i]).length === 0 && $(squareCombinations[i]).not(clickedDiv).length === 0) {
      ...
      // for(var i = 0; i < squareCombinations.length; i++) {
      //     spliceArray.push(i);
      //     if (squareCombinations[i].indexOf(this) == -1){
      //         console.log('Removing:' + squareCombinations[i] + '(' + spliceArray[i] + ')');
      //         squareCombinations.splice([i], 1);
      //         i--;
      //     }
      // };


      //squareCombinations.splice(i)
    }
  }
